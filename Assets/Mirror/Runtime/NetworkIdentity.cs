    ...
    public sealed class NetworkIdentity : MonoBehaviour {
        ...
        void CreateNetworkBehavioursCache() {
            if (_allowChildrenBehaviours) {
                List<NetworkBehaviour> networkBehavioursInChildren = GetComponentsInChildren<NetworkBehaviour>(true).ToList();

                networkBehavioursInChildren.Concat(GetNetworkBehavioursInGrandChildren(transform));

                networkBehavioursCache = networkBehavioursInChildren.ToArray();
            } else
        ...
        NetworkBehaviour[] GetNetworkBehavioursInGrandChildren(Transform grandParent) {
            List<Transform> gChilds = new List<Transform>();
            for (int i = 0; i < transform.childCount; i++) {
                gChilds.Add(transform.GetChild(i));
            }

            List<NetworkBehaviour> netBList = new List<NetworkBehaviour>();

            foreach (Transform transform in gChilds) {
                foreach (NetworkBehaviour netB in transform.GetComponentsInChildren<NetworkBehaviour>(true)) {
                    netBList.Add(netB);
                }
            }

            return netBList.ToArray();
        }

        NetworkIdentity[] GetNetworkIdentitiesInGrandChildren(Transform grandParent) {
            List<Transform> gChilds = new List<Transform>();
            for (int i = 0; i < transform.childCount; i++) {
                gChilds.Add(transform.GetChild(i));
            }

            List<NetworkIdentity> netIList = new List<NetworkIdentity>();

            foreach (Transform transform in gChilds) {
                foreach (NetworkIdentity netI in transform.GetComponentsInChildren<NetworkIdentity>(true)) {
                    netIList.Add(netI);
                }
            }

            return netIList.ToArray();
        }
        ...
        void OnValidate() {
            ...
            if (_allowChildrenBehaviours) {
                List<NetworkIdentity> netIds = GetComponentsInChildren<NetworkIdentity>(true).ToList();
                netIds.Concat(GetNetworkIdentitiesInGrandChildren(transform));
                foreach (NetworkIdentity ni in netIds) {
                    if (ni.gameObject != gameObject)
                        Debug.Log("Child object " + ni.gameObject.name + " cannot have child NetworkIdentities when using AllowChildrenBehaviours.");
                }
            }
#endif
        }
